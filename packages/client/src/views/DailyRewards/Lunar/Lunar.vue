...
<script>
import LunarDailyRewards from "@/lunar_daily_rewards";
...
export default {
  ...
  data: () => ({
    rewards: LunarDailyRewards.lunarRewards,
    timer: new Timer(true)
  }),
  computed: {
    ...mapState({
      step: state => state.dailyLunarReward.step,
      timeLeft: state => state.dailyLunarReward.timeLeft,
      collected: state => !state.dailyLunarReward.readyToCollect
    })
  },
  watch: {
    timeLeft: {
  ...
  methods: {
    ...
    async collect() {
      let items = await this.performRequest(this.$game.collectDailyLunarReward());
      if (!Array.isArray(items)) {
        items = [items];
      }
      await this.$game.notifications.updateDailyLunarReward();
